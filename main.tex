...
\begin{example}
    Consider the Z-transform function given by \todo{keep here or in background?}
    
    \begin{equation}
        H(z) = \frac{(z - j)(z + j)}{\left(z - \left(-\frac{1}{4} - \frac{1}{2}j\right)\right)\left(z - \left(\frac{1}{2} + \frac{1}{2}j\right)\right)}
    \end{equation}
    
    The ROC of $H(z)$ must exclude the poles at $z = -\frac{1}{4} - \frac{1}{2}j$ and $z = \frac{1}{2} + \frac{1}{2}j$, thus the ROC is $|z| > \frac{1}{2}$. The system is stable as the ROC includes the unit circle.
    ...
    \begin{figure}[H]
        ...
        \begin{subfigure}{.25\linewidth}
          ...
          \caption{r = 0.71 (2 d.p.)}
        \end{subfigure}\hfill
        \begin{subfigure}{.25\linewidth}
          \includegraphics[width=\linewidth]{images/valid_contour.png}
          \caption{r = 0.9}
        \end{subfigure}\hfill
        
        \caption{Illustration of Circular Contours: Displaying the effect of the radius $r$ on the contour ($n = 100$). The red dots represent the poles of $H(z)$.}
    \end{figure}
