...
The Z-transform generalizes Equation (\ref{dtft}) to the complex plane, not just the unit circle where $r = 1$ \citep{Oppenheim1989DTSP}.
...
\todo[inline]{more research needed to bridge the gap}

\noindent We can then expand upon the PMF, $p(x)$, to obtain the Probability Generating Function (PGF), $G_X(q)$, defined as
...
\todo[inline]{include diagram to show faster convergence time (illustration purposes)}
