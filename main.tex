...
The Z-transform generalises the discrete-time Fourier transform to the complex plane, not just the unit circle where $r = 1$. By setting $z = e^{i\omega}$ in the Z-transform, we obtain the DTFT (\ref{dtft}).

\subsection{Relation to Probability Discrete Functions}\label{pgfs}
Probability Discrete Functions (PDFs) are vital in the field of probability theory, providing a framework for the analysis of discrete random variables. In particular, the concept of the Probability Mass Function (PMF), which is defined for a discrete random variable $X$ taking on values $x_i$ with probabilities $p_i$, as $P(X=x_i) = p_i$. This brings forth the following conditions
\begin{equation}
	\sum_{i=0}^n p_i = 1 \hspace{2em}\text{and}\hspace{2em} 0 \leq p_i \leq 1 \hspace{0.5em}\forall i
\end{equation}

Expanding upon this, we introduce the Probability Generating Function (PGF), to encapsulate the entirety of a discrete random variable's distribution into a single function. For a random variable $X$ with PMF $p(x)$, the PGF, $G_X(z)$, is given by
\begin{equation}
	G_X(z) = E[z^X] = \sum^{\infty}_{x = 0} p(x)z^x,
\end{equation}
where $E[\cdot]$ denotes the expectation operator, and $z$ is a complex number. At first glance, we take note of the similarity between the PGF and the $z$-transform in Equation (\ref{bilateral_z-transform}). We can see a direct correspondence in their structures, essentially with the $z$-transform acting as a PGF for sequences indexed by integers.

As we proceed, the utility of these mathematical constructs in facilitating our understanding of complex phenomena will become more apparent, setting the groundwork for exploring the inverse $z$-transform and their applications.
