...
\\ \text{(Surah An-Najm, No. 53, Ayat 39)}
\end{center}
...
\\\\
I'd also like to extend my thanks to my dear brother and sister, outer family and friends for their continuous encouragement, support and motivation.
\\\\
To Carolyn Phelan - I am profoundly grateful for the continuous support and guidance provided during my final year of study. I would also like to extend my thanks to Guido Germano and Benjamin Loveless for their support through their works.
\\\\
...
\begin{abstract}
This study conducts a review on the inverse $z$-transform and its application in pricing discrete path-dependent options, a financial derivative whose payoffs depend on the path taken by the underlying asset price. Such options, known as exotic options, are the focus of this project, particularly lookback and barrier options. By examining various numerical approximation methods for the inverse $z$-transform, the research aims to evaluate their efficiency and accuracy. Whilst most methods are performed on a circular contour, we look into a conformal mapping resulting in a sinh deformation of the contour.
...
Similar to the approach in Section \ref{section:aw_speed}, we plot the results against different N values, where we set J = 2N, for the different implementations. Given that IFFT has a time complexity of O(NLogN), we expect this to be the dominating factor in Algorithm \ref{algo:cavers}. We take the average over multiple runs to smoothen significant interference of external processes.
...
The method introduced by \citet{Cavers1978FFT} is shown to achieve an accuracy surpassing 1E-16 but suffers from a high-computational cost. The method introduced by \citet{AbateWhitt1992a, AbateWhitt1992b} offers an accuracy of around 1E-11 but with a computational cost lower than that of Cavers; a middle ground showcasing the trade-off between accuracy and efficiency.	

% ========================================
% Critical Evaluation
% ========================================
\section{Critical Evaluation}
While we demonstrate success in finding parameters for the conformal mapping that resembles a circular contour, this success is only true for machine zero with double precision. The circular contour achieves a $L_1$ loss of 7E-31 and the parameter setup for the conformal mapping achieves a $L_1$ loss of 1.5E-19. The approach involved the use of gradient descent and the ADAM optimiser, but methods such as Bayesian optimisation, golden section search, and Quasi-Newton methods may offer an interesting approach. Despite the success of our parameter search, it was not possible to provide a foundation nor attempt utilising the sinh deformation in a format conducive to the circular contour methods.

Given the time constraints, presenting an even more thorough analysis of the approximation methods by \citet{AbateWhitt1992a, AbateWhitt1992b} and \citet{Cavers1978FFT}, as well as a more extensive test suite for the numerical benchmarking was not possible. The experiments conducted on \citet{Cavers1978FFT} could have been extended to all the transform pairs for both timings and the MAE of the DFT implementation rather than  base it on the results for the Heaviside step and Polynomial pairs. On the topic of the numerical benchmarks, while we offer one perspective in terms of the best accuracy achieved and the timings, it may also be beneficial to consider the worst error achieved and numerical values for the mean absolute error (MAE), similar to \citet{loveless2021guido}.

% ========================================
% Future Work
% ========================================
\section{Future Work}
Given the work in this study, we highlight several areas that could benefit from further research and development.

% ========================================
% Series Acceleration
% ========================================
\subsection{Series Acceleration}
Our experiments involved analysing the computational cost of the approximation methods and studying the trade-off between accuracy and efficiency. Whilst the method of \citet{Cavers1978FFT} offers high accuracy, the time complexity of the IFFT is costly and thus, we find it very interesting to see how the application of a series acceleration affects our results. 

\citet{smith1979acceleration} provide an in-depth analysis on different methods on the convergence of series for a wide array of problems; linear and logarithmic as well as linear and non-linear methods. \citet{loveless2023phelanguido} and \citet{fusai2016spitzer} show success in speeding up the convergence of the inverse $z$-transform via the Euler summation. The authors approximate Eqn. (\ref{inverse_z-transform}) by the binomial average of its partial sum and showcase its applicability to the pricing of double barrier options.

% ========================================
% Sinh Deformation Continuation
% ========================================
\subsection{Conformal Mapping Continuation}
As stated in Section \ref{section:sinh_deformation}, the lack of information adjusted the course of this project to the reconstruction of the unit circle. However, the continuation of the work by \citet{boyarchenko2024efficient} seems to be more promising. They introduce several new considerations as well as more examples, shedding light on the setup used. They state an increase in precision from E-10 to E-11, although it is not clear how they achieved this improvement; they discuss the bounds and formulas necessary to select appropriate parameters to ensure minimal truncation errors. 

% ========================================
% High-Performance Computing
% ========================================
\subsection{High-Performance Computing}
The implementation of NIZT methods on high-performance computing (HPC) platforms is another area for future exploration. Utilising parallel computing and specialised hardware can significantly speed up the calculations involved in the inverse $z$-transform. Transforming the methods into a form that enables the advantages of HPC could make them more practical for large-scale financial simulations and other applications requiring extensive computational resources. This would allow for the handling of more complex models and larger data, thereby broadening the scope and impact of the NIZT in various financial settings.
